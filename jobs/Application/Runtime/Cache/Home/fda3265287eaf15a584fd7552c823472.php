...
<div class="container well text-left" style="margin-left:auto;margin-right:auto;width:800px;">
	<?php if(is_array($list)): foreach($list as $key=>$vo): ?><h3><b>公司信息修改</b></h3>
    <h5>-----------------------------------------------------------------------</h5>
	<form action="<?php echo U('Home/Index/companyeditHandle');?>" method="post" name="creator">
		<label>公司名称：</label>
        <input type="text" name="rname" onblur="check_name()" class="span3 form-control" value="<?php echo ($vo["rname"]); ?>"> <div style="color:red;display:none;" id="user1">公司名称不能为空！</div></br>
        <label>联系方式：</label>
        <input type="text" name="phone" onblur="check_phone()" class="span3 form-control" value="<?php echo ($vo["phone"]); ?>"/><div style="color:red;display:none;" id="phone1">联系方式不能为空！</div></br>
		<label>公司简介:</label>
